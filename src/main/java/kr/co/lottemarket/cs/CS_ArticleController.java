...
public class CS_ArticleController {
	...
	public String noticeList(Model model, PageRequestDTO pageRequestDTO, int group, int cate1) {
		...
		if(pageRequestDTO.getGroup() != 2 ) {
			...
			List<ArticleDTO> articleLists = new ArrayList<ArticleDTO>();
			...
			for(ArticleDTO list : lists) {
				...
				int cate2Num = list.getCate2();

				log.info("cate2Num = " + cate2Num);

				List<ArticleDTO> articles = articleSerivce.selectFaqArticles(group, cate1,cate2Num);
